...
//  val a_block_vec = get_match_vec(io.a_req.bits, block_granularity)
  ...
  val mshrs_hash_sets = io.status.map(s => associativePolicy.get_hashed_index(Map("tag" -> s.bits.tag, "set" -> s.bits.set)))
  val c_req_hash_sets = associativePolicy.get_hashed_index(Map("tag" -> io.c_req.bits.tag, "set" -> io.c_req.bits.set))
  val b_req_hash_sets = associativePolicy.get_hashed_index(Map("tag" -> io.b_req.bits.tag, "set" -> io.b_req.bits.set))
  val a_req_hash_sets = associativePolicy.get_hashed_index(Map("tag" -> io.a_req.bits.tag, "set" -> io.a_req.bits.set))
