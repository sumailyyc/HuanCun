...
class Slice()(implicit p: Parameters) extends HuanCunModule {
  ...
  val sinkD_wset = associativePolicy.get_hashed_index(
    Map(
      "tag" -> sinkD.io.bs_waddr.bits.tag,
      "set" -> sinkD.io.bs_waddr.bits.set
    )
  )
  ...
  val sourceD_rset = associativePolicy.get_hashed_index(
    Map(
      "tag" -> sourceD.io.bs_raddr.bits.tag,
      "set" -> sourceD.io.bs_raddr.bits.set
    )
  )
  ...
  val sourceD_wset = associativePolicy.get_hashed_index(
    Map(
      "tag" -> sourceD.io.bs_waddr.bits.tag,
      "set" -> sourceD.io.bs_waddr.bits.set
    )
  )
...
//  dataStorage.io.sinkC_waddr <> ctrl_arb(sinkC.io.bs_waddr, ctrl.map(_.io.bs_w_addr))
  ...
  val sourceC_rset = associativePolicy.get_hashed_index(
    Map(
      "tag" -> sourceC_arb.bits.tag,
      "set" -> sourceC_arb.bits.set
    )
  )
  ...
  val sinkC_wset = associativePolicy.get_hashed_index(
    Map(
      "tag" -> sinkC_arb.bits.tag,
      "set" -> sinkC_arb.bits.set
    )
  )
